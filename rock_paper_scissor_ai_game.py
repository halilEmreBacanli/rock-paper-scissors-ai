...
def get_result(plr_ch):
    player_history.append(plr_ch)

    # ai guess
    ai_choice = choice[randint(0, 2)]  # default random

    if len(player_history) >= 3:
        # Strategy 1: If the number of games is more than 3 and the last move is repeated, try to beat it.
        if player_history[-1] == player_history[-2] == player_history[-3]:

            if player_history[-1] == 'rock':
                ai_choice = 'paper'
            elif player_history[-1] == 'paper':
                ai_choice = 'scissors'
            else:
                ai_choice = 'rock'

        # Strategy 2: If Strategy 1 didn't applied then find most used and try to beat it.
        else:
            r = player_history.count('rock')
            p = player_history.count('paper')
            s = player_history.count('scissors')
...
def main():
    ...
    total_without_tie = wins + loss

    if total_without_tie > 0:
        win_percentage = wins / total_without_tie * 100
        print("\nWin rate (excluding ties):", int(win_percentage), "%")
    else:
